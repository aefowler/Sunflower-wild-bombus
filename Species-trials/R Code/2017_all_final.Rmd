...
There are a number of bees that don't have Crithidia count data and also no death recorded or note about what happened to them. They appear to have been dissected 7 days after inoculation but they dont' have counts. None of the authors can remember what happened with these bees. They were removed from all analyses. 
...
# Analyze both species together: 

## Get set up 

```{r, results = 'hide', message = FALSE}
bees2017<-read_csv("CSV/2017_gris_bimacs.csv")
```
...
Sample size including dead bees: 210 
89 out of 210 bees died
Sample size excluding dead bees: 121 
...
Now lets get the means and SEs/CIs from the probability model 

Old method with predict function: (which does not work if your model includes REs)
```{r}
MCL <- 3.36
Treatment <- c("SF", "50-50", "WF")
...
# this gives us a fit and SE in the logit scale (not the response scale)
# which appears to be the exact same thing as emmean when we do not say type = response 
# so why dont we just use emmeans for all this?  
newdata2
```
...
New method with emmeans: 

```{r}
emmeans(gris_presence5,~Treatment)

gris_probs<-emmeans(gris_presence5,~Treatment)
gris_probs <- as.data.frame(gris_probs)
gris_probs$prob<- plogis(gris_probs$emmean)
gris_probs$LL<- plogis(gris_probs$emmean - (1.96 * gris_probs$SE))
gris_probs$UL<- plogis(gris_probs$emmean + (1.96 * gris_probs$SE))
